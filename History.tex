...
%=====================================
\section{Changes from 6.0 to 6.0.1}
\label{sec:history_60_to_601}

\begin{itemize}
\item Added the following examples for the 6.0 features:
\begin{itemize}
  \item Compound directive names (\specref{sec:compound})
  \item Updated OpenMP Affinity section to include affinity policy for free-agent threads
    (\specref{sec: affinity_policies})
  \item More transparent task examples
    (\specref{subsec:depend_trans_task})
  \item Tasking examples using free-agent threads (\specref{sec:free_agent})
  \item \kcode{taskgraph} construct for replay execution of taskgraph records.
    (\specref{sec:taskgraph})
  \item \kcode{self} modifier for the \kcode{map} clause to require self mapping
    (\specref{sec:self_mapping})
  \item \kcode{target_data} as a composite directive 
    (\specref{subsec:composite_target_data})
  \item \kcode{declare_target} directive with \kcode{local} clause 
    (\specref{subsec:declare_target_local})
   \item \kcode{workdistribute} construct with \kcode{target teams} construct 
    (\specref{sec:target_teams_workdistribute})
  \item \kcode{stripe} construct for changing the order of logical
    iterations of a loop nest (\specref{sec:stripe})
  \item \kcode{split} construct for loop splitting into multiple loops
    (\specref{sec:split})
  \item \kcode{fuse} construct for loop fusion (\specref{sec:fuse})
  \item Atomic compare-and-swap examples in Fortran
    (\specref{sec:cas})
  \item \kcode{safesync} clause for synchronization requirements within
    a thread team (\specref{sec:safesync})
  \item \kcode{groupprivate} directive for groupprivate variables
    (\specref{sec:groupprivate})
  \item \kcode{novariants} and \kcode{nocontext} clauses on
    \kcode{dispatch} construct (\specref{sec:dispatch})
\end{itemize}

\item Other changes:
\begin{itemize}
  \item Removed the \kcode{atomic update} directive that is not necessary
    in Example \example{workshare.3.f} (\specref{sec:workshare}).
  \item Updated text and examples that cover pointer attachment and pointer
    initialization for devices. Renamed \docref{Pointer Mapping} section
    to \docref{Pointer Initialization for Devices}
    (\specref{sec:pointer_mapping}).
  \item Fixed a logical condition test in checking the \kcode{user} trail
    for \kcode{metadirective} in Example \example{target_ptr_map.5.f90}
    (\specref{sec:pointer_mapping})
  \item Changed to use the underscore form of directive names for the 6.0
    examples. This effectively fixed the incorrect use of an undefined 
    \kcode{declare induction} for \kcode{declare_induction}
    (\specref{subsec:user-defined-induction}).
  \item Replaced the use of deprecated Fortran include file \bcode{omp_lib.h}
    with Fortran module \bcode{omp_lib} in a number of examples
    (\nspecref{Table}{tab:Updated Examples 6.0})
\end{itemize}
...
\begin{itemize}
  ...
  \item Added missing \kcode{declare target} directive for procedures
    called inside \kcode{target} region in Examples
  ...
  \item Added missing \kcode{end target} directive in Example
    \example{declare_mapper.3.f90} (\specref{sec:declare_mapper})
...
\begin{itemize}
  ...
  \item Fixed the placement of \kcode{declare simd} directive in 
    Examples \example{linear_modifier.*.f90} (\specref{sec:linear_modifier})
...
\begin{itemize}
  ...
  \item Examples to illustrate the ordering properties of 
     the \emph{flush} operation (\specref{sec:mem_model})
...
\item Added the following examples for the 5.1 features:
\begin{itemize}
  \item OpenMP directives in C++ \emph{attribute} specifiers
    (\specref{sec:attributes})
