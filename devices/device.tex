...
\subsection{\kcode{omp\_is\_initial\_device} Routine}
...
\subsection{\kcode{omp\_get\_num\_devices} Routine}
...
\subsection{\kcode{omp\_set\_default\_device} and \\
\kcode{omp\_get\_default\_device} Routines}
...
The following \kcode{omp_target_is_present} function returns \ucode{0}
(\plc{false}, of \bcode{integer(C_INT)} type) to indicate that \ucode{h_cp}
does not have any corresponding storage on the default device.
