...
The shape-operator can appear in either the \kcode{affinity} clause, the
\plc{data-motion clause} or the \kcode{depend} clause.

The following example shows the use of the shape-operator in the 
\kcode{target_update} directive. The shape-operator \ucode{([nx][ny+2])}
casts pointer variable \ucode{a} to a 2-dimensional array of size
\ucode{nx$\times$(ny+2)}.  The resulting array is then accessed as
...
data in the \kcode{target_update} directive.
