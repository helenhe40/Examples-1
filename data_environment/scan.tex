...
Basically, the \kcode{inscan} modifier connects a worksharing-loop and/or SIMD reduction to
...
before and after the directive, respectively) is to use an \emph{inclusive} or 
\emph{exclusive} scan value for the list item (\ucode{x}).

The first example uses the \emph{inclusive} scan operation on a composite
worksharing-loop SIMD construct. The \kcode{scan} directive separates the reduction
...
The second example uses the \emph{exclusive} scan operation on a composite
worksharing-loop SIMD construct. The \kcode{scan} directive separates the use of \ucode{x}
...
the concept of an \emph{initialization phase} where a private variable 
can be set for later use in the \emph{input phase} of
an \emph{exclusive} scan operation.
