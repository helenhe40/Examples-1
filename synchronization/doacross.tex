...
a \emph{doacross loop nest} where cross-iteration dependences exist.
A \kcode{doacross} clause on an \kcode{ordered} construct within an \emph{ordered}
loop describes the dependences of the \emph{doacross} loops. 
...
the \emph{doacross loop nest} extended to two nested loops, \ucode{i} and \ucode{j},
...
a \emph{doacross loop nest}.  The \ucode{i} and \ucode{j} loops are the associated
loops for the collapsed loop as well as for the \emph{doacross loop nest}.
