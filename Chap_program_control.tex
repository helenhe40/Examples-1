...
The \kcode{metadirective} and \kcode{declare_variant} directives provide conditional 
selection of directives and procedures for compilation (and use), respectively.
...
if the \kcode{OMP_DISPLAY_ENV} environment variable has been set to \vcode{TRUE} or \vcode{VERBOSE}. 
 ...
 %to be used in a \kcode{parallel} region.  It can be set with the \kcode{OMP_NUM_THREADS} environment variable, 
 %the \kcode{omp_set_num_threads()} API function, or the \kcode{num_threads} clause.  The default \plc{nthreads-var}
 %value is implementation defined.  All of the ICVs are presented in the \docref{Internal Control Variables} section
 %of the \docref{Directives} chapter of the OpenMP Specifications document.  Within the same document section, override 
...
Certain combinations of nested constructs are permitted, giving rise to \emph{combined} constructs
...
A \emph{composite} construct is a combined construct which has one or more clauses with (an often obviously)
