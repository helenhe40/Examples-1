...
substitution. They use \emph{context selectors} to specify the contexts in which
the variant may be selected for substitution. A context selector specifies
various \emph{trait selectors}, grouped into \emph{trait selector sets}. A trait
...
\emph{OpenMP context}. A context selector is considered to be \emph{compatible}
...
specify one or more \emph{trait properties}. Additionally, a trait selector may
optionally specify a \emph{trait score} for explicit control over variant
selection.
...
\emph{function variant}, to be used in place of the \emph{base function} 
...
for C/C++ in OpenMP Specification 5.1 to allow nesting of declare variant directives. 
...
In the third example, a \kcode{construct} selector set is specified in the
\kcode{when} clause. Here, a \kcode{metadirective} directive is used within a
procedure \ucode{exp_pi_diff} that is annotated with a declare target
directive. The program expects the procedure to be called from one of two
contexts. It can be called from within a \kcode{target teams} region or it can
be called \emph{outside} a \kcode{target} region but from an implicit or
explicit parallel region. However, this usage of the \kcode{target} trait
selector is not advisable. If the \kcode{target teams} region executes on the
host device instead of a target device (e.g., due to offloading being disabled
via the \kcode{OMP_TARGET_OFFLOAD} environment variable or due to lack of
available target devices), the \emph{target} trait may not be set for the
called procedure if the implementation generates only a single version of the
procedure for execution on the host device that may also be called from outside
a \kcode{target} region. In general, a program cannot portably rely on the
declare target directive to set the \emph{target} trait when a procedure is
called from a \kcode{target} region if the region executes on the host device.

\cexample[5.2]{metadirective}{3a}

\ffreeexample[5.2]{metadirective}{3a}

Rather than relying on the \emph{target} trait with a metadirective, a better
approach would be to condition the directives on whether the call is made from
inside a \kcode{teams} or \kcode{parallel} region. Ideally, there would be a
way to implicitly generate different function variants from a single procedure
definition, each to be called in some specified context. The defined procedure
could then specialize further using a metadirective. Unfortunately, OpenMP
does not currently provide such a mechanism. Alternatives would be to create
specialized versions of the procedure for the different contexts and utilize
the declare variant directive. Metadirectives might still be useful with this
approach to avoid code duplication, but may also require use of the
preprocessor (an option that is not generally available for Fortran).

In the following C/C++ example, the procedure \ucode{exp_pi_diff} is defined in
its own header file. At the point the definition is included, a \kcode{match}
clause may be defined via the preprocessor, and if it is defined then
definition will be included with corresponding \kcode{begin declare_variant}
and \kcode{end declare_variant} directives around it. The declare variant
directives effectively import context information into the definition, which
can then be used by a metadirective (or another call to a procedure with a
declare variant directive). The \bcode{_Pragma} operator is used in the header
file to portably allow for macro expansion within the directive.

\begin{ccppspecific}
\srcnexample[6.0]{metadirective}{3b}{h}
\end{ccppspecific}
\cexample[6.0]{metadirective}{3b}
...
In the \ucode{bar} procedure metadirectives are nested.
At the outer level a selection between serial and parallel execution is performed
