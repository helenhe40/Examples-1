...
\kcode{declare_variant} directive, with matching requirements specified
...
In Case 7, the \kcode{novariants} clause disables variant
substitution for the call to \ucode{foo()}. Normally the  \kcode{nocontext} clause 
directs the implementation to not include the \kcode{dispatch} construct, but since 
variant substitution is already disabled, the base function \ucode{foo()} will be 
called.
