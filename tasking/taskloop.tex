...
the explicit \kcode{taskgroup} construct in the example ensures that the function does not exit
...
hence the reduction sum is executed \ucode{T$\times$N} times.
...
In the example, the code thus prints \pout{x1 = 16384} (\ucode{T$\times$N}) and
