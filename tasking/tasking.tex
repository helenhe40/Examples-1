...
the \docref{Task Scheduling} section of the OpenMP Specification affect the usage of
...
The following two examples demonstrate how the scheduling rules illustrated in 
\docref{Task Scheduling} section of the OpenMP Specification affect the usage of locks
...
In the following example, \ucode{lock} is held across a task scheduling point. 
 However, according to the scheduling restrictions, the executing thread cannot
...
the parent task for the task inside \ucode{foo} if the task is \plc{included}
or \plc{undeferred}.
Thus, the result of the execution may differ depending on whether the task is merged 
or not. Therefore the \kcode{mergeable} clause needs to be used with caution. In this example,
the use of the \kcode{mergeable} clause is safe. As \ucode{x} is a shared variable the
