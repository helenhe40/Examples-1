...
omitted, then the tasks could execute in any order and the program
...
\ucode{N$\times$N} elements, and the multiplication is implemented using blocks
of \ucode{BS$\times$BS} elements.
...
is produced by \ucode{x} with an \kcode{in} dependence type in the
...
(While unnecessary, the \kcode{depend(inout: \ucode{y})} clause on the second child task is
...
Note: While a \kcode{taskwait} construct ensures that all child tasks have completed; a \kcode{depend} clause on a \kcode{taskwait}
...
In OpenMP 5.1 the \kcode{omp_all_memory} \emph{reserved locator} was introduced
...
\index{transparent clause@\kcode{transparent} clause}
\index{clauses!transparent@\kcode{transparent}}
\index{OpenMP identifiers!omp_import@\kcode{omp_import}}
\index{OpenMP identifiers!omp_export@\kcode{omp_export}}
\index{OpenMP identifiers!omp_impex@\kcode{omp_impex}}

Dependences of child tasks of a task can be exposed by specifying 
the task as a \emph{transparent task} with the 
\kcode{transparent(\plc{impex-type})} clause.
The \plc{impex-type} argument indicates that the task is 
an importing (\kcode{omp_import}), exporting (\kcode{omp_export}),
or both importing and exporting (\kcode{omp_impex}) task.
For the purposes of dependence matching, an importing task is 
one that makes its preceding tasks (such as earlier
sibling tasks) visible to its child tasks and an exporting task
is one that makes its child tasks visible to its successors.
The default is \kcode{omp_impex} if the argument is not explicitly specified.

In the following example, task T2 is specified as a transparent task
with the \kcode{transparent} clause. This makes the dependence on
variable \ucode{x} from task T1 visible to task T3 (importing) and 
the dependence on variable \ucode{x} from task T3 visible to task T4
(exporting).  Without the \kcode{transparent} clause on T2, T3 would not
wait on T1, T4 would not wait on T3, and the value of \ucode{x} in T4 
would be undefined due to race condition.

\cexample[6.0]{task_dep}{14}
\ffreeexample[6.0]{task_dep}{14}

In the following example,
each iteration of the \ucode{h}-loop updates all
...
\kcode{transparent(omp_impex)} clause.  As a result of the exposed
...
\cexample[6.0]{task_dep}{15}
\ffreeexample[6.0]{task_dep}{15}
