...
In the OpenMP Specification 4.0, by default, all variables within the lexical
scope of the construct are copied \emph{to} and \emph{from} the
device, unless the device is the host, or the data exists on the
device from a previously executed data-type construct that
has created storage on the device and possibly copied host
...
specifies the device storage and data movement \emph{to} and \emph{from} the
device, and controls on the storage duration.

There is an important change in the OpenMP Specification 4.5
...
In the OpenMP Specification version 4.5 the mechanism for target
execution is specified as occurring through a \emph{target task}.
...
transfers have finished. Using the \kcode{nowait} clause allows asynchronous
execution of the \kcode{target} region.
...
\input{devices/target_teams_workdistribute}
