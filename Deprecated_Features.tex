...
6.0 & Fortran include file \kcode{omp_lib.h}
    & Fortran module \kcode{omp_lib} \\
\hline
...
\nolinenumbers
\dpftable{6.0}
\begin{supertabular}{p{2.1in} p{1.0in} p{2.0in}}
  \hexentry{udr.1}[f90]{4.0} &
    \plc{combiner} expression in \kcode{declare} \\
  \hexentry{udr.2}[f90]{4.0} & 
    \kcode{reduction} directive changed to \\
  \hexentry{udr.3}[f90]{4.0} & use \kcode{combiner} clause \\
  \hexentry[f90]{udr.4}{4.0} & \\
  \hexentry[cpp]{udr.5}{4.0} & \\
  \hexentry[cpp]{udr.6}{4.0} & \\[2pt]
\hline\\[-2ex]
  \hexentry[f]{default_none.1}{pre-3.0$^*$} & 
     Fortran include file \kcode{omp_lib.h} \\
  \hexentry[f90]{fort_loopvar.1}{pre-3.0$^*$} & 
     replaced with Fortran module \\
  \hexentry[f]{directive_syntax_F_fixed_comment.1}{pre-3.0$^*$} & 
     \kcode{omp_lib} \\
  \hexentry[f90]{fort_race.1}{pre-3.0$^*$} & \\
  \hexentry[f90]{mem_model.1}{3.1$^*$} & \\
  \hexentry[f]{mem_model.2}{3.1$^*$} & \\
  \hexentry[f]{mem_model.3}{3.1$^*$} & \\
  \hexentry[f]{collapse.3}{3.0$^*$} & \\
  \hexentry[f]{get_nthrs.1}{pre-3.0$^*$} & \\
  \hexentry[f]{get_nthrs.2}{pre-3.0$^*$} & \\
  \hexentry[f]{nthrs_dynamic.1}{pre-3.0$^*$} & \\
  \hexentry[f]{nthrs_dynamic.2}{pre-3.0$^*$} & \\
  \hexentry[f]{parallel.1}{pre-3.0$^*$} & \\
  \hexentry[f]{set_dynamic_nthrs.1}{pre-3.0$^*$} & \\
  \hexentry[f]{simple_lock.1}{pre-3.0$^*$} & \\
  \hexentry[f90]{tasking.10}{3.0$^*$} & \\[2pt]
\hline\\[-2ex]
  \multicolumn{3}{l}{\emph{$^*$Version tag is unchanged since
  the replacement feature is available in the earlier version.}}\\[2pt]
\end{supertabular}
